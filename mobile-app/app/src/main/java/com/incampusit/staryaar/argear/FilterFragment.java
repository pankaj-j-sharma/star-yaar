...
public class FilterFragment
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        if (getActivity() != null) {
            mContentsViewModel = new ViewModelProvider(getActivity(), new MyViewModelFactory(this.getActivity().getApplication(), "MyParam")).get(ContentsViewModel.class);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.clear_filter_button: {
                ((ArGearCameraActivity) getActivity()).clearFilter();
                break;
            }
            case R.id.filter_plus_button:
                ((ArGearCameraActivity) getActivity()).setFilterStrength(10);
                break;
            case R.id.filter_minus_button:
                ((ArGearCameraActivity) getActivity()).setFilterStrength(-10);
                break;
            case R.id.vignett_button:
                ((ArGearCameraActivity) getActivity()).setVignette();
                break;
            case R.id.blur_button:
                ((ArGearCameraActivity) getActivity()).setBlurVignette();
                break;
        }
    }
    ...
    public void onFilterSelected(int position, ItemModel item) {
        ((ArGearCameraActivity) getActivity()).setFilter(item);
    }
}
